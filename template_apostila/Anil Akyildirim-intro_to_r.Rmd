...
1.  What command would you use to extract just the counts of girls baptized? Try
    it!
**Answer 1. The command i would use to extract just the counts of girst baptized is below**
...
**Answer 2. Trend in the number of girls baptized over the years?**
Starting from year ~ 1660, there seems to be a positive linear correlation and trend between years and girls baptized. As the years are passing, the number of girls baptized are increasing starting from year 1660. However it looks like the relationship has changed over the years as prior to year 1660; there wasnt a definite trend between over the years. Between, years 1640 and 1650, there seems to be a negative linear relationship over the years as the amount of girls baptized declines within these years.
...
**Answer 3. Proportion of Boys over time**

```{r plot-proportion-of-the-boys-vs-year, eval=TRUE}
...
**Answers. Last Section**

First view the dataset.
...
The years included in the data set is below - 1940 to 2002-
...
Dimensions of the data frame is 63 rows and 3 columns
...
Column names are , "years", "boys", "girls"
...
Yes they are. The arbuthnot's data set had 82 rows with 3 columns, present data set has the same amount of columns, with same column values with 63 rows. They are similar scale.
...
Present data set boy to girl ratio plot below.

```{r plot-boy-to-girl ratio-vs-year, eval=TRUE}
plot(present$year, (present$boys / present$girls), type = "l")
```
Arbuthnot data set boy to girl ration plot below.

```{r plot-prop-boys-to-girls-arbuthnot, eval=TRUE}
plot(arbuthnot$year, (arbuthnot$boys / arbuthnot$girls), type = "l")
```
What do you see? Does Arbuthnot's observation about boys being born in
    greater proportion than girls hold up in the U.S.? Include the plot in your 
    response.

Comparing the two plots, we see that both Arbuthnot's observation and Present data set has boys being born in greater proportions than girls. See two plots. 
...
First we can add the total births per year and create the total column.
...
We can further filter the present data set to see the row that has the max birth. That is year 1961. 
